...
main_(int argc, char **argv) {
	...
	while (fgets(line, 128, fp)) {
		...
		sscanf(line, "%[^/]/%lud", addr_str, &mask);
		if (inet_aton(addr_str, &addr) == 0) { //invalid address
		...
		//00011000 00000000 00000000 00000000 Mask
		//00000000 10110010 01001101 11001011 IP(revert order) 203.77.178.0
//		printf("IP: %.04x ---- Mask: %.04x\n", p->p_key, p->p_m->pm_mask);
	...
	char ip[16] = "45.118.136.0";
	struct in_addr i_ip;
	inet_aton(ip, &i_ip);
	struct ptree* snode = pat_search(i_ip.s_addr, phead);
	if(!snode){
		printf("Node not found\n");
	}else{
//		printf("IP: %.04x, Mask length: %d\n", snode->p_key, snode->p_mlen);
	}
	exit(0);
}